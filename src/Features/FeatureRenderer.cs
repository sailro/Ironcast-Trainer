...
internal abstract class FeatureRenderer : ToggleFeature
{
	...
	internal class SelectionContext
	{
		public SelectionContext(IFeature feature, OrderedProperty orderedProperty, float parentX, float parentY, Func<object, IPicker> builder, SelectionContextType contextType)
		{
			...
			Picker = builder(orderedProperty.Property.GetValue(feature, null));
	...
	private bool HandleSelectionContext(SelectionContext context)
	{
		...
		property.SetValue(context.Feature, picker.RawValue, null);
	...
	private void RenderFeatureWindow(int id)
	{
		...
		var features = Context
			.Features.Where(f => f is not FeatureRenderer)
			.ToArray();

		var tabs = fixedTabs
			.Concat(features.Select(RenderFeatureText))
			.ToArray();
		...
		switch (_selectedTabIndex)
		{
			case 0:
				...
				break;
			default:
				var feature = features[_selectedTabIndex - fixedTabs.Length];
	...
	private static string RenderFeatureText(Feature feature)
	{
		...
		return string.Format(Strings.CommandStatusTextFormat, feature.Name, toggleFeature.Enabled ? Strings.TextOn : Strings.TextOff, string.Empty);
	}
	...
	{
		ConfigurationManager.Save(Context.ConfigFile, Context.Features);
	}
	...
	protected void LoadSettings(bool warnIfNotExists = true)
	{
		...
		ConfigurationManager.Load(Context.ConfigFile, Context.Features, warnIfNotExists);
		ControlValues.Clear();
	...
	private void RenderFeatureProperty(Feature feature, OrderedProperty orderedProperty)
	{
		...
		var currentValue = property.GetValue(feature, null);
		...
		if (currentValue != newValue && property.CanWrite)
			property.SetValue(feature, newValue, null);
				...
				if (currentValue is IFeature subFeature)
				{
					...
					if (length > 0)
					{
						...
						foreach (var innerOrderedProperty in subProperties)
						{
							...
							var innerPropertyValue = innerProperty.GetValue(subFeature, null);
